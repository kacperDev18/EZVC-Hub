...
local P,M=W:CreateTab("Play"),W:CreateTab("Macro")
...
local function wj(p,d)
    ...
    local o,s=pcall(function()return HS:JSONEncode(d)end)
    if not o then return false end
    return pcall(writefile,p,s)
end
local function rj(p)
    if type(isfile)~="function" or type(readfile)~="function" or not isfile(p)then return nil end
    local o,d=pcall(readfile,p);if not o then return nil end
    local k,a=pcall(function()return HS:JSONDecode(d)end)
    if not k or type(a)~="table" then return nil end
    return a
end
local function lf()
    ...
    for _,p in ipairs{FO,"tdmacro","./tdmacro/"}do
        local o,l=pcall(listfiles,p)
        if o and type(l)=="table" then for _,f in ipairs(l)do local x=tostring(f):match("([^/\\]+)%.json$");if x and x~="state" and x~="config" and not sn[x]then sn[x]=true;n[#n+1]=x end end end
    end
    return n
end
local function rd(d,o)
    if not d then return end
    if type(d.Refresh)=="function" then pcall(d.Refresh,d,o)end
    if type(d.SetOptions)=="function" then pcall(d.SetOptions,d,o)end
end
...
local D = {
    diff="Easy Mode", gs="1", mn="", sel="",
    sk=false, rp=false, lb=false, st=false,
    rec=false, pl=false
}
...
local rec = {a={},i=1,n=1,t={},k={},l=0,active=false}
local buf = nil  -- loaded macro buffer for playback
...
local function nm(o)
    if type(o)=="string" or type(o)=="number" then return tostring(o)end
    if type(o)=="table" then return tostring(o.Name or o.Value or o[1]or"")end
...
task.spawn(function()
    ...
    while true do
        ...
        else if lf2 then lf2=false end end
        task.wait(.2)
    end
end)
...
task.spawn(function()
    local last=0
    while true do
        if Cfg.st and fB("found") and tick()-last>3 then
            local b = fB(Cfg.diff)
            if b then
                if getconnections then pcall(function() for _,c in ipairs(getconnections(b.MouseButton1Click)) do if c.Fire then c:Fire() end end end) end
                pcall(function() b.MouseButton1Click:Fire() end)
            end
            task.wait(.2)
            local b2 = fB("found")
            if b2 then
                if getconnections then pcall(function() for _,c in ipairs(getconnections(b2.MouseButton1Click)) do if c.Fire then c:Fire() end end end) end
                pcall(function() b2.MouseButton1Click:Fire() end)
                last=tick(); C.L=C.L+1; rfC()
            end
        end
        task.wait(.5)
    end
    ...
    while true do
        if buf and #buf>0 then
            ...
            local function nf(p)
                ...
                local bd,b=20,nil
                for _,x in ipairs(tw:GetChildren())do if not P2.k[x]then
                ...
                if a then
                    ...
                    if a.t=="P" then
                        ...
                        if PL then
                            ...
                            local t0=tick()
                            while tick()-t0<2 do local i=nf(cf.Position); if i then P2.t[a.i]=i; P2.k[i]=a.i; break end; task.wait(.05) end
                        end
                    elseif a.t=="U" then
                        local UP=ens("u","RemoteFunctions","UpgradeTower")
                        local i = P2.t[a.i]; if UP and i and i.Parent then pcall(function() UP:InvokeServer(i) end) end
                    elseif a.t=="S" then
                        local SE=ens("s","RemoteFunctions","SellTower")
                        local i = P2.t[a.i]; if SE and i and i.Parent then pcall(function() SE:InvokeServer(i) end); P2.t[a.i]=nil; P2.k[i]=nil end
                    elseif a.t=="W" then
                        local SK=ens("w","RemoteEvents","SkipWaveVote"); if SK then pcall(function() SK:FireServer(1) end) end
                    elseif a.t=="G" then
                        local SP=ens("g","RemoteEvents","SetGameSpeed"); if SP then pcall(function() SP:FireServer(a.v) end) end
                    end
                end
                P2.i = P2.i + 1
            end
...
-- ============ PLAY TAB ============
P:CreateDropdown({Name="Difficulty",
    Options={"Easy Mode","Medium Mode","Hard Mode","Insane Mode","Crazy Mode"},
    CurrentOption=Cfg.diff,
    Callback=function(o) set("diff",nm(o)) end})

P:CreateDropdown({Name="Game Speed",Options={"1","1.50","2"},CurrentOption=tostring(Cfg.gs),
    Callback=function(v)
...
P:CreateToggle({Name="Auto Skip Wave",CurrentValue=Cfg.sk,Callback=function(v)
    set("sk", v)
    if v then lastW=nil; setVis(true) else setVis(false) end
end})

P:CreateToggle({Name="Auto Replay",CurrentValue=Cfg.rp,Callback=function(v)
    set("rp", v)
    if v then startRP() else rpL=false end
end})

P:CreateToggle({Name="Auto Lobby",CurrentValue=Cfg.lb,Callback=function(v) set("lb", v) end})

P:CreateToggle({Name="Auto Start Map",CurrentValue=Cfg.st,Callback=function(v) set("st", v) end})

-- ============ MACRO TAB ============
M:CreateInput({Name="Macro Name",CurrentValue=Cfg.mn,PlaceholderText="np. EasyFarm",
    RemoveTextAfterFocusLost=false,
    Callback=function(t) set("mn", t or "") end})
...
sDD = M:CreateDropdown({Name="Saved Macros",Options=lf(),CurrentOption=Cfg.sel or "",
    Callback=function(o) sel = nm(o); set("sel", sel) end})

M:CreateToggle({Name="Record Macro",CurrentValue=Cfg.rec,Callback=function(v)
    set("rec", v)
    if v then
        if Cfg.sel=="" then N("TD","Najpierw wybierz macro"); return end
        rec.a={}; rec.i=1; rec.n=1; rec.t={}; rec.k={}; rec.l=tick(); rec.active=true
        ...
        if #rec.a>0 and Cfg.sel~="" then
            ...
            N("TD","Nic nie nagrano")
        end
    end
end})

M:CreateButton({Name="Load Macro",Callback=function()
    if Cfg.sel=="" then N("TD","Wybierz macro"); return end
    local a = rj(FO..Cfg.sel..".json")
    if not a or #a==0 then N("TD","Puste lub brak: "..Cfg.sel); return end
    buf = a
    N("TD","Załadowano: "..Cfg.sel.." ("..#a..")")
end})
...
M:CreateButton({Name="Refresh",Callback=function()
    local f = lf(); rd(sDD, f); N("TD","Znaleziono "..#f.." macro")
end})

M:CreateToggle({Name="Play Macro",CurrentValue=Cfg.pl,Callback=function(v)
    set("pl", v)
    if v then
        if buf then return end
        if Cfg.sel=="" then N("TD","Najpierw Load Macro"); return end
        local a = rj(FO..Cfg.sel..".json")
        if not a or #a==0 then N("TD","Pusty macro"); return end
        buf = a
    end
end})

M:CreateButton({Name="Clear",Callback=function()
    buf = nil
    N("TD","Cleared")
end})
...
task.spawn(function()
    ...
    -- Game Speed (pozwól grze załadować RemoteEvent)
    local gs = tonumber(Cfg.gs)
    if gs then
        local g = ens("g","RemoteEvents","SetGameSpeed")
        if g then
            local o = pcall(function() g:FireServer(gs) end)
            if o then print("[CFG] Applied Game Speed: "..tostring(gs)) end
        end
    end

    -- Auto Skip visual
    if Cfg.sk then lastW=nil; setVis(true); print("[CFG] Auto Skip resumed") end

    -- Auto Replay
    if Cfg.rp then startRP(); print("[CFG] Auto Replay resumed") end

    -- Selected macro w dropdownie
    if sDD and Cfg.sel~="" then pcall(function() sDD:Set(Cfg.sel) end) end

    -- Play Macro (jeśli był ON, wczytaj + odtwórz)
    if Cfg.pl and Cfg.sel~="" then
        local a = rj(FO..Cfg.sel..".json")
        if a and #a>0 then buf = a; print("[CFG] Play Macro resumed") end
    end

    print("[CFG] loaded:", Cfg.diff, Cfg.gs, Cfg.mn, "sk="..tostring(Cfg.sk), "rp="..tostring(Cfg.rp), "lb="..tostring(Cfg.lb), "st="..tostring(Cfg.st))
end)
